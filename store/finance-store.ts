...
export const useFinanceStore = create<FinanceState>()(
  persist(
    (set, get) => ({
      ...
      getAvailableBalance: () => {
        const { transactions, projects, expectedTransactions } = get();
        ...
        const allocatedToProjects = projects.reduce((sum, p) => sum + p.currentAmount, 0);
        
        // Consider expected transactions that are not paid yet
        const expectedExpenses = expectedTransactions
          .filter(t => !t.isPaid && t.type === 'expense')
          .reduce((sum, t) => sum + t.amount, 0);
        
        const expectedIncome = expectedTransactions
      ...
      getTotalHouseholdIncome: () => {
        ...
        return profile.householdMembers
          .filter(member => member.type === 'adult' && member.salary)
          .reduce((sum, member) => sum + (member.salary || 0) + (member.commissions || 0), 0);
      },
